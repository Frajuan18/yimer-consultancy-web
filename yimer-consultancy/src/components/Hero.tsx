...
export const Hero: React.FC = () => {
  ...
  return (
    <section className={`relative min-h-screen poppins pt-32 lg:pt-40 ${isDark ? 'bg-black' : 'bg-white'}`}>
      {/* Background */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-start">
          ...
          <div className="text-left pt-8">
            ...
            {/* Description */}
            <p className={`
              text-base sm:text-lg mb-10 max-w-lg leading-relaxed
            ...
            <div className="space-y-3 mb-10">
              {features.map((feature, idx) => (
                ...
                >
                  <div className={`
                    relative p-4 rounded-xl transition-all duration-300 ease-out cursor-pointer
                    ${hoveredFeature === idx 
                      ? (isDark 
                          ? 'bg-white/20 scale-[1.02]' 
                          : 'bg-gray-100 scale-[1.02] shadow-sm')
                      : (isDark 
                  ...
                  `}>
                    <div className="flex items-start gap-3">
                      <feature.icon className={`w-5 h-5 mt-0.5 transition-colors duration-200 ${isDark ? 'text-blue-400' : 'text-blue-600'}`} />
                      <div className="flex-1">
                        <div className="flex items-center justify-between">
                          <h3 className={`font-semibold transition-colors duration-200 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                            {feature.title}
                          </h3>
                          <ChevronRight className={`w-4 h-4 transition-all duration-300 ease-out ${hoveredFeature === idx ? 'translate-x-1 opacity-100' : 'opacity-40'} ${isDark ? 'text-white/40' : 'text-gray-400'}`} />
                        </div>
                        <p className={`text-sm mt-1 transition-colors duration-200 ${isDark ? 'text-white/60' : 'text-gray-600'}`}>
                          {feature.description}
                        </p>
                        
                        {/* Expanded Info on Hover */}
                        {hoveredFeature === idx && (
                          <div className={`mt-3 pt-3 border-t transition-all duration-300 ease-out animate-fade-in ${isDark ? 'border-white/10' : 'border-gray-200'}`}>
                            <p className={`text-xs mb-2 ${isDark ? 'text-white/70' : 'text-gray-700'}`}>
                              {feature.details}
                            </p>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 mb-12">
              <button className="group relative px-8 py-3 rounded-lg overflow-hidden text-sm font-semibold transition-all duration-200 hover:scale-105 active:scale-95">
                ...
                <span className={`relative z-10 flex items-center gap-2 transition-all duration-300 font-semibold ${isDark ? 'text-gray-900' : 'text-white'}`}>
                  Get Started
          ...
          <div className="lg:sticky lg:top-32">
                ...
                <div className="space-y-3 mb-6">
                  {[
                    { 
                      ...
                      desc: 'Market analysis & strategic planning', 
                      icon: Target,
                      details: 'Custom roadmaps for sustainable growth'
                    },
                    { 
                      title: 'Business Analytics', 
                      desc: 'Data-driven decision making', 
                      icon: LineChart,
                      details: 'Predictive analytics & KPI tracking'
                    },
                    { 
                      title: 'Operations', 
                      desc: 'Process optimization & efficiency', 
                      icon: TrendingUp,
                      details: 'Streamline workflows & reduce costs'
                    },
                    { 
                      title: '24/7 Support', 
                      desc: 'Round-the-clock assistance', 
                      icon: Headphones,
                      details: 'Dedicated team with rapid response'
                    },
                  ].map((service, idx) => (
                    <div
                      key={idx}
                      className={`
                        group p-3 rounded-xl transition-all duration-300 ease-out cursor-pointer
                        ${isDark 
                    ...
                    >
                      <div className="flex items-start gap-3">
                        <service.icon className={`w-4 h-4 mt-0.5 transition-colors duration-200 ${isDark ? 'text-blue-400' : 'text-blue-600'}`} />
                        <div className="flex-1">
            ...
            {/* Floating badge */}
            <div className={`
              absolute -top-3 right-3 lg:-top-5 lg:-right-5
